...
def simple_descriptors(I: np.ndarray, Y, X, n_bins: int = 16, radius: int = 40, sigma: int = 2):
    """
...
def display_matches(I1: np.ndarray, pts1: np.ndarray, I2: np.ndarray, pts2: np.ndarray, show: bool = True):
    """
...
#   1: I_yy
#   2: I_xy 
def get_2nd_derivatives(I_x: np.ndarray, I_y: np.ndarray, sigma: float) -> list:
    G = gauss(sigma)
    D = gaussdx(sigma)
    D = np.flip(D)
    # G_T = G.reshape([G.shape[1], 1])
